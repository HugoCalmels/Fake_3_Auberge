...
export async function getAdminBookings(): Promise<AdminBookingDto[]> {
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Impossible de recuperer les reservations.");
  }
...
export async function getAdminBookingById(
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Impossible de recuperer la reservation.");
  }
...
export async function createAdminBooking(
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Impossible de creer la reservation.");
  }
...
export async function updateAdminBooking(
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Impossible de modifier la reservation.");
  }
...
export async function cancelAdminBooking(
  ...
  const response = await fetch(getApiUrl(`admin/bookings/${bookingId}/cancel`), {
    method: "PATCH",
    headers: getAdminAuthHeaders(),
  });

  if (!response.ok) {
    throw await parseApiError(response, "Impossible d'annuler la reservation.");
  }
...
export async function assignAdminBookingRoom(
  ...
  const response = await fetch(getApiUrl(`admin/bookings/${bookingId}/assign-room`), {
    method: "PATCH",
    headers: getAdminAuthHeaders(),
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Impossible de reassigner la chambre.");
  }
