...
import { SelectedRoomConfig } from "@/features/booking/types";
...
export default function BookingPaymentStep({
  ...
  const mealPlanLabel = selectedRoom
    ? selectedRoom.mealPlans.find(
        (mealPlan) => mealPlan.code === selectedRoom.mealPlanCode,
      )?.name ?? selectedRoom.mealPlanCode
    : "—";
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
      ...
      <div className="rounded-[22px] border border-[#d8d0c2] bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-2 text-[14px] text-[#5f584d]">
          ...
          <InfoRow
            label="Voyageurs"
            value={selectedRoom ? `${selectedRoom.persons} voyageur(s)` : "—"}
          />
          <InfoRow label="Formule" value={mealPlanLabel} />
          <InfoRow label="Sous-total chambre" value={`${roomSubtotal} EUR`} />
          <InfoRow label="Supplements" value={`${supplementsTotal} EUR`} />
        </div>
